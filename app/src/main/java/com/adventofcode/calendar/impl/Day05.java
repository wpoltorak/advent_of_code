...
public class Day05 implements Puzzle {
    ...
    private static final String CONFIGURATION_AND_INSTRUCTIONS_SEPARATOR = System.lineSeparator()
    ...
    private String printCrates(Map<Integer, List<Integer>> stacks) {
        ...
        // IntStream mapToInt = stacks.values().stream().mapToInt(s ->
        // s.stream().mapToInt(Integer::valueOf).reduce((first, second) -> second)
        return "Top crates: " + str;
    }

    private Map<Integer, List<Integer>> createStacks(String[] config) {
        Map<Integer, List<Integer>> stacks = createEmptyStacks(
                config[config.length - 1].trim().split(STACK_NUMBER_SEPARATOR));
